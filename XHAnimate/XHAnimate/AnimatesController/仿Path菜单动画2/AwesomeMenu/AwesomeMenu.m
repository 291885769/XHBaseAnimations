...
@synthesize nearRadius, endRadius, farRadius, timeOffset, rotateAngle, menuWholeAngle, startPoint, expandRotation, closeRotation, animationDuration, rotateAddButton,menuGrapyType;
...
- (id)initWithFrame:(CGRect)frame startItem:(AwesomeMenuItem*)startItem menuItems:(NSArray *)menuItems
{
    ...
    if (self) {
        ...
        self.menuGrapyType=SDiffuseMenuGrapyTypeArc;
...
- (void)_setMenu {
    ...
    for (int i = 0; i < count; i ++)
    {
        ...
        if (self.menuGrapyType==SDiffuseMenuGrapyTypeLine) { // 直线形
            
            float x           = startPoint.x;
            float y           = startPoint.y + i * (endRadius / (count + 1));
            CGPoint endPoint    =  CGPointMake(x,y);
            item.endPoint   = RotateCGPointAroundCenter(endPoint,  startPoint,   rotateAngle - (M_PI));
            
            x               = startPoint.x;
            y               = startPoint.y + i * (nearRadius / (count + 1));
            CGPoint nearPoint   =  CGPointMake(x,y);
            item.nearPoint  = RotateCGPointAroundCenter(nearPoint,startPoint, rotateAngle - (M_PI));
            
            x               = startPoint.x;
            y               = startPoint.y + i * (farRadius / (count + 1));
            CGPoint farPoint    =  CGPointMake(x,y);
            item.farPoint   = RotateCGPointAroundCenter(farPoint,  startPoint, rotateAngle - (M_PI));
